...
const OUTFIT_STYLES = ['Beach Getaway', 'City Explorer', 'Festival Vibes', 'Garden Party', 'Summer Night Out', 'Casual Hangout'];
...
function App() {
    ...
    const handleGenerateClick = async () => {
        ...
        OUTFIT_STYLES.forEach(style => {
            initialImages[style] = { status: 'pending' };
        });
        setGeneratedImages(initialImages);

        const concurrencyLimit = 2; // Process two at a time
        const stylesQueue = [...OUTFIT_STYLES];

        const processStyle = async (style: string) => {
            try {
                const prompt = `Reimagine the person in this photo in a summer outfit suitable for a "${style}" theme. This includes clothing, accessories, and hairstyle, fitting the occasion. The background should complement the outfit. The output must be a photorealistic image showing the person clearly.`;
                const resultUrl = await generateStyledImage(uploadedImage, prompt, style);
    ...
    const handleDownloadIndividualImage = (style: string) => {
        ...
        if (image?.status === 'done' && image.url) {
            ...
            link.href = image.url;
            link.download = `summer-style-snap-${style.toLowerCase().replace(/ /g, '-')}.jpg`;
    ...
    const handleDownloadAlbum = async () => {
        ...
        try {
            ...
            if (Object.keys(imageData).length < OUTFIT_STYLES.length) {
            ...
            link.download = 'summer-style-snap-album.jpg';
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
        ...
        <main className="bg-black text-neutral-200 min-h-screen w-full flex flex-col items-center justify-center p-4 pb-24 overflow-hidden relative">
            ...
            <div className="z-10 flex flex-col items-center justify-center w-full h-full flex-1 min-h-0">
                <div className="text-center mb-10">
                    <h1 className="text-6xl md:text-8xl font-caveat font-bold text-neutral-100">Summer Style Snap</h1>
                    <p className="font-permanent-marker text-neutral-300 mt-2 text-xl tracking-wide">Try on different summer looks with AI.</p>
                </div>
                     ...
                     <div className="relative flex flex-col items-center justify-center w-full">
                        ...
                        >
                            ...
                            <p className="mt-8 font-permanent-marker text-neutral-500 text-center max-w-xs text-lg">
                                Click the polaroid to upload your photo and start your summer style session.
                            </p>
                        </motion.div>
                    </div>
                )}
