 ...
 * @param style The outfit style string (e.g., "Beach Getaway").
 * @returns The fallback prompt string.
 */
function getFallbackPrompt(style: string): string {
    return `Create a photograph of the person in this image as if they were dressed for a "${style}" summer occasion. The photograph should capture distinct fashion, accessories, and an overall atmosphere suitable for that theme. Ensure the final image is a clear photograph that looks authentic.`;
}
 ...
 * @param style The specific style theme (e.g., 'Beach Getaway') to be used in the fallback prompt.
 * @returns A promise that resolves to a base64-encoded image data URL of the generated image.
 */
export async function generateStyledImage(imageDataUrl: string, prompt: string, style: string): Promise<string> {
